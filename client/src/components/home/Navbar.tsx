      ...
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex-1 max-w-2xl mx-8 relative">
          <div>
           ...
           {search?.length > 0  && (
  <div
    className="absolute top-full left-0 bg-white shadow-lg border w-full mt-1 rounded max-h-80 overflow-y-auto z-50"
  >
    {search.map((item) => (
      <div
        key={item.id}
   ...
   <div className="text-center py-2 border-t">
      <button onClick={handleNavigate} className="text-blue-600 cursor-pointer hover:underline">
        See all results
      </button>
   </div>
  </div>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Avatar className="w-8 h-8">
                    ...
                    <AvatarFallback className="border bg-gray-300 border-gray-300 p-2 cursor-pointer rounded-full">{user.name
    ?.trim()
    .split(/\s+/)
    .map(word => word[0])
    .join('')
      ...
      <DropdownMenuContent className="w-56" align="start">
        ...
        <DropdownMenuItem>
           ...
           <button onClick={handleLogout} className="cursor-pointer">Logout</button>
            ...
            </div> : <Link href='/auth/login' className="flex items-center gap-2 text-gray-700 hover:text-gray-900 cursor-pointer">
